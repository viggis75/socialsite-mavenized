...
public class ProfileHandler extends RestrictedDataRequestHandler {
    ...
    public Future<?> get(SocialRequestItem request) {
        ...
        if (request.getUsers() != null && request.getUsers().size() > 0) {
            ...
            try {
                ...
                if (profile == null) {
                    ...
                    log.warn(msg);
                    return ImmediateFuture.newInstance(new ResponseItem(HttpServletResponse.SC_BAD_REQUEST, msg));
        ...
        return ImmediateFuture.newInstance(new ResponseItem(
                HttpServletResponse.SC_BAD_REQUEST, "No user specified"));
    }
    ...
    public Future<? extends ResponseItem> post(SocialRequestItem request) {
        ...
        try {
            ...
            try {
                if (!userManager.hasRole("admin", viewerId)) {
                    return ImmediateFuture.newInstance(new ResponseItem(
                        HttpServletResponse.SC_UNAUTHORIZED, "Only admins can create new profiles"));
                }
            } catch (UserManagementException ex) {
                log.error("ERROR determining user role", ex);
                return ImmediateFuture.newInstance(new ResponseItem(
        ...
        if (request.getUsers() != null && request.getUsers().size() > 0) {
            ...
            try {
                ...
                if (!viewerId.equals(ownerId) && !userManager.hasRole("admin", viewerId)) {
                    return ImmediateFuture.newInstance(new ResponseItem(
                        HttpServletResponse.SC_UNAUTHORIZED, "Only owner or admin can update a profile"));
                }
            } catch (UserManagementException ex) {
                log.error("ERROR determining user role", ex);
                return ImmediateFuture.newInstance(new ResponseItem(
            ...
            try {
                ...
                if (profile == null) {
                    String msg = String.format("Cannot find userId=%s", ownerId);
                    log.warn(msg);
                    res = new ResponseItem(HttpServletResponse.SC_BAD_REQUEST, msg);
                }
            ...
            res = new ResponseItem(HttpServletResponse.SC_BAD_REQUEST, "No user specified");
        }
        log.trace("END - ERROR");
        return ImmediateFuture.newInstance(res);
    }
