...
public abstract class RestrictedDataRequestHandler { 
    ...
    public static void authorizeRequest(SocialRequestItem request) throws SocialSpiException{
        ...
        try {
            ...
            throw new SocialSpiException(HttpServletResponse.SC_UNAUTHORIZED, e.getMessage());

        } catch (Exception e) {
            if (log.isErrorEnabled()) {
                log.error("Unexpected Failure", e);
            }
            throw new SocialSpiException(HttpServletResponse.SC_BAD_REQUEST, e.getMessage());
        }
        log.trace("END");
    }
}
